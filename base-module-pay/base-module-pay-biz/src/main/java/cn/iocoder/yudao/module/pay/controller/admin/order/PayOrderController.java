...
public class PayOrderController {
    ...
    public void exportOrderExcel(@Valid PayOrderExportReqVO exportReqVO,
        ...
        if (excelList!=null && excelList.size()>0){
            ...
            for(PayOrderExcelVO dd:excelList){
                if (StringUtil.isNotEmpty(dd.getMerchantOrderId())){
                    orderIds.add(Long.valueOf(dd.getMerchantOrderId()));
                }
            }
            List<TradeOrderRespDTO> orderList = tradeOrderApi.getOrderList(orderIds);
            Map<Long,TradeOrderRespDTO> orderMap = convertMap(orderList,TradeOrderRespDTO::getId);
            for(PayOrderExcelVO dd:excelList){
                if (StringUtil.isNotEmpty(dd.getMerchantOrderId())){
                    Long orderId = Long.valueOf(dd.getMerchantOrderId());
                    if (orderMap.containsKey(orderId)){
                        TradeOrderRespDTO order = orderMap.get(orderId);
                        dd.setLogisticsCompany(order.getLogisticsCompany());
                        dd.setLogisticsNo(order.getLogisticsNo());
                    }
                }
            }
        }
