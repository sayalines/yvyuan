...
public class PayChannelServiceImpl implements PayChannelService {
    ...
    @Override
    public PayChannelRespVO getChannelByCode(String code) {
        LambdaQueryWrapper<PayChannelDO> wrapper = new LambdaQueryWrapper<>();
        wrapper = wrapper.eq(PayChannelDO::getCode,code);
        List<PayChannelDO> list = payChannelMapper.selectList(wrapper);
        if (list!=null && list.size()>0){
            return PayChannelConvert.INSTANCE.convert(list.get(0));
        }
        return null;
    }
