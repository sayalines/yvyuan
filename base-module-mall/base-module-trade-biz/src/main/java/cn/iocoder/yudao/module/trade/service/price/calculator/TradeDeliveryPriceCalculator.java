...
public class TradeDeliveryPriceCalculator implements TradePriceCalculator {
    ...
    private void calculateExpress(TradePriceCalculateReqBO param, TradePriceCalculateRespBO result) {
        ...
        //是否含包邮商品
        if (result.getFreeShipping()!=null && result.getFreeShipping()){
            return;
        }
        ...
        if (CollUtil.isEmpty(expressTemplateMap)) {
            log.info("[calculate][找不到商品 templateIds {} areaId{} 对应的运费模板]", deliveryTemplateIds, address.getAreaId());
//            throw exception(PRICE_CALCULATE_DELIVERY_PRICE_TEMPLATE_NOT_FOUND);
    ...
    private boolean isGlobalExpressFree(TradePriceCalculateRespBO result) {
        ...
        // 开启包邮
        if (config != null && Boolean.TRUE.equals(config.getDeliveryExpressFreeEnabled())){
            Integer deliveryExpressFreePrice = config.getDeliveryExpressFreePrice();
            Integer deliveryExpressFreeQty = config.getDeliveryExpressFreeQty();
            // 满足包邮的价格或者件数
            if ((deliveryExpressFreeQty>0 && result.getRealProductCount()>=deliveryExpressFreeQty) ||
                    (deliveryExpressFreePrice>0 && result.getPrice().getPayPrice() >=deliveryExpressFreePrice)){
                res = true;
            }
