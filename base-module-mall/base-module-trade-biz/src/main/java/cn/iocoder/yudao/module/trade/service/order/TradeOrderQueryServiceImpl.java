...
public class TradeOrderQueryServiceImpl implements TradeOrderQueryService {
    ...
    @Override
    public Long getOrderCount(Long userId, AppTradeOrderPageReqVO reqVO) {
        Long result = tradeOrderMapper.selectCount(reqVO, userId);
        if (result==null){
            result = 0L;
        }
        return result;
    ...
    @Override
    public Long getOrderCount(Long userId, Integer status, Boolean commentStatus,Integer notType) {
        return tradeOrderMapper.selectCountByUserIdAndStatus(userId, status, commentStatus,notType);
    }
    ...
    @Override
    public Integer findPayProductCountByUserIdAndSpuId(Long userId, Long spuId) {
        Integer result = tradeOrderMapper.findPayProductCountByUserIdAndSpuId(userId,spuId);
        if (result==null){
            result = 0 ;
        }
        return result;
    ...
    public Boolean isNewUser(Long userId) {
        ...
        wrapper = wrapper.eq(TradeOrderDO::getUserId,userId).eq(TradeOrderDO::getPayStatus,true);
        Long count = tradeOrderMapper.selectCount(wrapper);
    ...
    public TradeOrderDO getOrderByNo(String no) {
        ...
        return tradeOrderMapper.selectOne(TradeOrderDO::getNo,no);
    }

    @Override
    public List<TradeOrderDO> findList(LambdaQueryWrapper<TradeOrderDO> wrapper) {
        return tradeOrderMapper.selectList(wrapper);
    }

    @Override
    public Long getAllOrderCount(Long userId) {
        LambdaQueryWrapper<TradeOrderDO> wrapper = new LambdaQueryWrapper<TradeOrderDO>()
                .eq(TradeOrderDO::getUserId, userId);
        Long result = tradeOrderMapper.selectCount(wrapper);
        if (result==null){
            result = 0L;
        }
        return result;
    ...
    @Override
    public Long getDeliverOrderCount(Long userId, Integer status) {
        LambdaQueryWrapper<TradeOrderDO> wrapper = new LambdaQueryWrapper<TradeOrderDO>()
                .eq(TradeOrderDO::getUserId, userId)
