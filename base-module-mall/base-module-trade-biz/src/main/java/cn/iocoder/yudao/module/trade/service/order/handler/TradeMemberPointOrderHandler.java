...
import cn.iocoder.yudao.module.member.api.config.TaskConfigApi;
...
public class TradeMemberPointOrderHandler implements TradeOrderHandler {
    ...
    @Override
    public void afterRefundOrder(TradeOrderDO order) {
        // 增加（回滚）用户积分（订单抵扣）
        reducePoint(order.getUserId(), order.getGivePoint(), MemberPointBizTypeEnum.ORDER_USE_CANCEL, order.getId());
        // 增加（回滚）用户经验
        memberLevelApi.reduceExperience(order.getUserId(), order.getPayPrice(), MemberExperienceBizTypeEnum.ORDER_GIVE_CANCEL.getType(), String.valueOf(order.getId()));
    }
