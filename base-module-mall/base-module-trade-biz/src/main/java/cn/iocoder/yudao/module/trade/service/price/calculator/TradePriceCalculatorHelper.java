...
public class TradePriceCalculatorHelper {
    ...
    public static TradePriceCalculateRespBO buildCalculateResp(TradePriceCalculateReqBO param,
        ...
        result.setRealProductCount(0);
        result.setExchangeCode(param.getExchangeCode());
        result.setFreeShipping(param.getFreeShipping());
        ...
        param.getItems().forEach(item -> {
            ...
            //购买的商品数量
            result.setProductCount(result.getProductCount()+item.getCount());
            if (item.getIsGift()==null || !item.getIsGift()){
                // 实际购买商品数量（不含赠品）
                result.setRealProductCount(result.getRealProductCount()+item.getCount());
            }
            ...
            orderItem.setDiscountPrice(0).setDeliveryPrice(0).setCouponPrice(0).setPointPrice(0).setVipPrice(0);
            // sku 信息
            ...
            orderItem.setSpuName(spu.getName()).setCategoryId(spu.getCategoryId())
                    .setDeliveryTemplateId(param.getDeliveryTemplateId())
