...
public class TradeConfigServiceImpl implements TradeConfigService {
    ...
    public void saveTradeConfig(TradeConfigSaveReqVO saveReqVO) {
        ...
        if (dbConfig != null) {
            TradeConfigDO entity = TradeConfigConvert.INSTANCE.convert(saveReqVO).setId(dbConfig.getId());
            entity.setDeliveryExpressFreeQty(saveReqVO.getDeliveryExpressFreeQty());
            tradeConfigMapper.updateById(entity);
            return;
        }
        // 不存在，则进行插入
        TradeConfigDO entity =TradeConfigConvert.INSTANCE.convert(saveReqVO);
        entity.setDeliveryExpressFreeQty(saveReqVO.getDeliveryExpressFreeQty());
