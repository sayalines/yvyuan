...
public class CartServiceImpl implements CartService {
    ...
    public Long addCart(Long userId, AppCartAddReqVO addReqVO) {
        ...
        //判断购买商品是否在限时购买时间内
        ProductSpuRespDTO spuRespDTO = productSpuApi.getSpu(sku.getSpuId());
        if (spuRespDTO!=null){
            if (spuRespDTO.getIsLimitTime()!=null && spuRespDTO.getIsLimitTime()){
                if (spuRespDTO.getLimitTimeStart()!=null){
                    if (spuRespDTO.getLimitTimeStart().isAfter(LocalDateTime.now())){
                        throw exception(SPU_PRODUCT_LIMNIT_NO_START);
                    }
                }

                if (spuRespDTO.getLimitTimeEnd()!=null){
                    if (spuRespDTO.getLimitTimeEnd().isBefore(LocalDateTime.now())){
                        throw exception(SPU_PRODUCT_LIMNIT_END);
                    }
                }
            }
        }
