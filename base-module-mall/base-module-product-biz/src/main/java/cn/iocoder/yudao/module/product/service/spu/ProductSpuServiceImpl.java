...
public class ProductSpuServiceImpl implements ProductSpuService {
    ...
    public Long createSpu(ProductSpuCreateReqVO createReqVO) {
        ...
        spu.setGiveGift(createReqVO.getGiveGift());
        spu.setIsLimitTime(createReqVO.getIsLimitTime());
        spu.setLimitTimeStart(createReqVO.getLimitTimeStart());
        spu.setLimitTimeEnd(createReqVO.getLimitTimeEnd());
        spu.setLimitCount(createReqVO.getLimitCount());
        spu.setIsFirstProduct(createReqVO.getIsFirstProduct());
        // 初始化 SPU 中 SKU 相关属性
    ...
    public void updateSpu(ProductSpuUpdateReqVO updateReqVO) {
        ...
        updateObj.setGiveGift(updateReqVO.getGiveGift());
        updateObj.setIsLimitTime(updateReqVO.getIsLimitTime());
        updateObj.setLimitTimeStart(updateReqVO.getLimitTimeStart());
        updateObj.setLimitTimeEnd(updateReqVO.getLimitTimeEnd());
        updateObj.setLimitCount(updateReqVO.getLimitCount());
        updateObj.setIsFirstProduct(updateReqVO.getIsFirstProduct());
        updateObj.setFreeShipping(updateReqVO.getFreeShipping());
        ...
        //判断规格是否有货,更新预约提醒时间
        List<ProductSkuDO> tmpSkuList =productSkuService.getSkuListBySpuId(updateReqVO.getId());
        if (tmpSkuList!=null && tmpSkuList.size()>0){
            List<Long> tmpIds = new ArrayList<>();
            for(ProductSkuDO dd:tmpSkuList){
                if (dd.getStock()!=null && dd.getStock()>0){
                    tmpIds.add(dd.getId());
                }
            }
            messageRemindApi.updateYydhRecord(tmpIds);
        }
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateSpu(ProductSpuDO updateReqVO) {
        // 校验 SPU 是否存在
        validateSpuExists(updateReqVO.getId());

        productSpuMapper.updateById(updateReqVO);
    }

    @Override
    public void validSkuBeforeDelete(Long id) {
        if (id!=null){
...
//        }
    ...
    @Override
    public PageResult<ProductUnionVo> getUnionProductPage(Integer pageNo, Integer pageSize, String keyword,Boolean isFirstProduct) {
        PageResult<ProductUnionVo> pageResult = new PageResult<>();
        Long total = productSpuMapper.findUnionProductCount(keyword,isFirstProduct);
        List<ProductUnionVo> list = new ArrayList<>();
        if (total.compareTo(0L)>0){
            Integer startIndex = (pageNo-1)*pageSize;
            list = productSpuMapper.findUnionProduct(startIndex,pageSize,keyword,isFirstProduct);
            if (list==null){
                list=new ArrayList<>();
            }
        }
    ...
    public void updateSpuStock(Map<Long, Integer> stockIncrCounts) {
        ...
        stockIncrCounts.forEach((id, incCount) -> productSpuMapper.updateSalesCount(id, -1*incCount));
    }
