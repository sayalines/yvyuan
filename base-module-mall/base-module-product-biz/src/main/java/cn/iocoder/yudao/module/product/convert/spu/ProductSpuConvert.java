...
public interface ProductSpuConvert {
    ...
    default ProductSpuDetailRespVO convertForSpuDetailRespVO(ProductSpuDO spu, List<ProductSkuDO> skus) {
        List<ProductSkuRespVO> skuList= ProductSkuConvert.INSTANCE.convertList(skus);
        if (skuList!=null && skuList.size()>0){
            for(ProductSkuRespVO dd:skuList){
                for(ProductSkuDO d2:skus){
                    if (dd.getId().compareTo(d2.getId())==0){
                        dd.setAllocateCount(d2.getAllocateCount());
                        break;
                    }
                }
            }
        }
        ProductSpuDetailRespVO vo = convert03(spu).setSkus(skuList);
        vo.setGiveGift(spu.getGiveGift());
        vo.setIsLimitTime(spu.getIsLimitTime());
        vo.setLimitTimeStart(spu.getLimitTimeStart());
        vo.setLimitTimeEnd(spu.getLimitTimeEnd());
        vo.setLimitCount(spu.getLimitCount());
        vo.setIsFirstProduct(spu.getIsFirstProduct());
        vo.setFreeShipping(spu.getFreeShipping());
        return vo;
    }
