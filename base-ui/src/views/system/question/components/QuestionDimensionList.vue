...
<!--      <el-table-column label="简介" align="left" prop="description" />-->
      <el-table-column label="分值计算类型" align="center" prop="scoreType" width="120px">
  ...
  <!-- 表单弹窗：添加/修改 -->
  <QuestionDimensionForm ref="formRef" @success="getList" />
</template>
...
import QuestionDimensionForm from './QuestionDimensionForm.vue'
import {DICT_TYPE} from "@/utils/dict";

defineOptions({ name: 'QuestionDimension' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await QuestionDimensionApi.getQuestionDimensionPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await QuestionDimensionApi.deleteQuestionDimension(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await QuestionDimensionApi.exportQuestionDimension(queryParams)
    download.excel(data, '量表维度.xls')
  } catch {
  } finally {
