      ...
      <el-row>
        ...
        <el-col :span="24">
          <el-form-item label="选择因子" prop="factors">
            <el-select v-model="formData.factors" multiple placeholder="请选择" style="width: 100%">
              <el-option
        ...
        <el-col :span="24">
          <el-form-item label="配置参数" prop="optContent">
            ...
            >
              ...
              <el-table-column align="center" label="指导语" min-width="120">
                <template #default="{ row }">
                  <el-input v-model="row.comment" type="textarea" rows="4"  class="w-100%" />
                </template>
              </el-table-column>
...
import * as QuestionDimensionApi from '@/api/system/questiondimension'
...
const open = async (type: string, id?: number, questionId: number) => {
  ...
  factorList.value = await QuestionFactorApi.getQuestionFactorList({questionId:questionId})
  // 修改时，设置数据
  if (id) {
    formLoading.value = true
    try {
      formData.value = await QuestionDimensionApi.getQuestionDimension(id)
      let array = []
...
const submitForm = async () => {
  ...
  try {
    ...
    const data = formData.value as unknown as QuestionDimensionApi.QuestionDimensionVO
    if (formType.value === 'create') {
      await QuestionDimensionApi.createQuestionDimension(data)
      message.success(t('common.createSuccess'))
    } else {
      await QuestionDimensionApi.updateQuestionDimension(data)
