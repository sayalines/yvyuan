  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="用户IP" prop="userIp">
        <el-input v-model="formData.userIp" placeholder="请输入用户IP" />
      </el-form-item>
      <el-form-item label="手机型号" prop="mobileModel">
        <el-input v-model="formData.mobileModel" placeholder="请输入手机型号" />
      </el-form-item>
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await VisitLogApi.getVisitLog(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as VisitLogApi.VisitLogVO
    if (formType.value === 'create') {
      await VisitLogApi.createVisitLog(data)
      message.success(t('common.createSuccess'))
    } else {
      await VisitLogApi.updateVisitLog(data)
