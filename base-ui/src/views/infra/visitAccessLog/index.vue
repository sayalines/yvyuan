<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="执行时长" align="center" prop="duration" width="100">
        <template #default="scope"> {{ formatPast2(scope.row.duration) }} </template>
      </el-table-column>
  ...
  <!-- 表单弹窗：详情 -->
  <VisitAccessLogDetail ref="detailRef" />
</template>
...
import VisitAccessLogDetail from './VisitAccessLogDetail.vue'

defineOptions({ name: 'InfraVisitAccessLog' })
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, '页面访问日志.xls')
  } catch {
  } finally {
