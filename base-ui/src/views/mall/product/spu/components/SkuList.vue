<template>
  ...
  >
    ...
    <el-table-column align="center" label="已分配数量" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.allocateCount" :min="0" class="w-100%" readonly="readonly" />
      </template>
    </el-table-column>
  ...
  >
    ...
    <el-table-column align="center" label="已分配数量" min-width="80">
      <template #default="{ row }">
        {{ row.allocateCount }}
      </template>
    </el-table-column>
...
const deleteSku = async(row) => {
  //删除规格前校验
  await  SpuApi.validSkuBeforeDelete(row.id);
...
const generateTableData = (propertyList: any[]) => {
  ...
  for (const item of buildSkuList) {
    const row = {
      properties: item != undefined ? (Array.isArray(item) ? item : [item]) : [], // 如果只有一个属性的话返回的是一个 property 对象
      price: 0,
      marketPrice: 0,
      costPrice: 0,
      barCode: '',
