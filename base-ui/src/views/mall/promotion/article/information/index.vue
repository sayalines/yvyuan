<template>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="200">
        <template #default="scope">
          ...
          <el-button
            ...
            link
            type="primary"
            @click="createLinkIn(scope.row.id)"
          >
            生成二维码
          </el-button>
          <el-button
  ...
  <!-- 表单弹窗：添加/修改 -->
  <Information ref="formRef" @success="getList" />
</template>
...
import Information from './informationForm.vue'
...
/** 生成二维码 */
const createLinkIn = async (id: number) => {
  try {
    // 二次确认
    await message.confirm("是否生成二维码")
    // 生成链接
    await ArticleApi.createLinkIn(id)
...
onMounted(async () => {
  ...
  if (Array.isArray(categoryTree.value)) {
    // 查找 id 为 20 的节点及其子树
    const subtree = treeData.value.find(node => node.id === 20) || {
      id: 20,
      children: [] // 如果没有找到 id 为 20 的节点，返回一个空的子树
    };
    if (subtree.children) {
      categoryTree.value = subtree.children;
    } else {
      // 如果没有子节点，返回当前节点
      categoryTree.value = [subtree];
    }
  } else {
