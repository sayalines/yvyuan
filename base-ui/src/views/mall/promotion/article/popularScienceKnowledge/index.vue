<template>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="200">
        <template #default="scope">
          ...
          <el-button
            ...
            link
            type="primary"
            @click="createLinkPo(scope.row.id)"
          >
            生成二维码
          </el-button>
          <el-button
  ...
  <!-- 表单弹窗：添加/修改 -->
  <PopularScience ref="formRef" @success="getList" />
</template>
...
import PopularScience from "./popularScienceKnowledgeForm.vue";
import { reactive,computed} from 'vue';

defineOptions({ name: 'popularScienceKnowledge' })
...
/** 生成二维码 */
const createLinkPo = async (id: number) => {
  try {
    // 二次确认
    await message.confirm("是否生成二维码")
    // 生成链接
    await ArticleApi.createLinkPo(id)
...
// 确保 categoryTree.value 是一个数组
  if (Array.isArray(categoryTree.value)) {
    // 查找 id 为 8 的节点及其子树
    const subtree = treeData.value.find(node => node.id === 8) || {
      id: 8,
      children: [] // 如果没有找到 id 为 8 的节点，返回一个空的子树
    };
    // categoryTree.value 现在包含以 id 为 8 的节点及其子节点
