...
<!--          <el-radio :key="3" :label="3">新人券</el-radio>-->
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="发放时间" prop="releaseTimes">
        <el-date-picker
          v-model="formData.releaseTimes"
          :default-time="[new Date(2000, 1, 1, 0, 0, 0), new Date(2000, 2, 1, 23, 59, 59)]"
          style="width: 240px"
          type="datetimerange"
          value-format="x"
      ...
      <el-form-item label="图片" prop="picUrl">
        <UploadImgs v-model:modelValue="formData.picUrl" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const formRules = reactive({
  ...
  releaseTimes: [{ required: true, message: '领取时间不能为空', trigger: 'change' }],
})
  ...
  if (id) {
    ...
    try {
      ...
      formData.value = {
        ...
        releaseTimes: [data.releaseStartTime, data.releaseEndTime]
      }
      // 获得商品范围
    ...
    const data = {
      ...
      validEndTime:
        formData.value.validTimes && formData.value.validTimes.length === 2
          ...
          : undefined,
      releaseStartTime:
        formData.value.releaseTimes && formData.value.releaseTimes.length === 2
          ? formData.value.releaseTimes[0]
          : undefined,
      releaseEndTime:
        formData.value.releaseTimes && formData.value.releaseTimes.length === 2
          ? formData.value.releaseTimes[1]
          : undefined
