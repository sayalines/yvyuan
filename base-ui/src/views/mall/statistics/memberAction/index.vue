...
<!--        />-->
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['statistics:memberAction:export']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      <el-table-column label="访问页面" align="center" prop="title" />
...
<!--      <el-table-column label="页面曝光点击率" align="center" prop="exposureRate" />-->
      <el-table-column label="新访客数" align="center" prop="newUserCount" />
      <el-table-column label="老访客数" align="center" prop="oldUserCount" />
    </el-table>
    <!-- 分页组件 -->
    <Pagination
      :total="total"
...
import {formatDate, isTodyDay} from "@/utils/formatTime";
import download from '@/utils/download'
import {DICT_TYPE, getBoolDictOptions} from "@/utils/dict";
defineOptions({ name: 'MemberActionStatistics' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await MemberStatisticsApi.getMemberActionPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await MemberStatisticsApi.exportMemberActionExcel(queryParams)
    download.excel(data, '访客行为.xls')
  } catch {
  } finally {
