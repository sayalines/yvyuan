...
<!--        />-->
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['statistics:productDetail:export']"
        >
...
defineOptions({ name: 'ProductDetailStatistics' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ProductStatisticsApi.getProductDetailPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await ProductStatisticsApi.exportProductDetailExcel(queryParams)
    download.excel(data, '商品明细.xls')
  } catch {
  } finally {
