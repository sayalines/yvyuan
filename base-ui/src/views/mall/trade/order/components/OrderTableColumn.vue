        ...
        <el-table-column min-width="300" prop="spuName">
          <template #header>
            <div
              ...
              style="background-color: #f7f7f7;user-select: text"
            >
              <span class="mr-20px" style="width:200px">订单ID：{{ scope.row.id }} </span>
              <span class="mr-20px" style="width:230px">订单号：{{ scope.row.no }} </span>
              <span class="mr-20px">下单时间：{{ formatDate(scope.row.createTime) }}</span>
              <span class="mr-20px" style="width:120px">用户ID：{{ scope.row.userId }} </span>
<!--              <span>订单来源：</span>-->
<!--              <dict-tag :type="DICT_TYPE.TERMINAL" :value="scope.row.terminal" class="mr-20px" />-->
              <span>支付状态：</span>
              <span v-if="scope.row.payStatus" class="mr-20px">已支付</span>
