<template>
                ...
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item
                    v-if="
                      ...
                      row.status === TradeOrderStatusEnum.COMPLETED.status
                    "
                    command="refund">
                    <Icon icon="ep:money" />
                    退款
  ...
  <OrderRefundForm ref="refundForm" @success="getList" />
</template>
...
import OrderRefundForm from '@/views/mall/trade/order/form/OrderRefundForm.vue'
...
const dynamicSearchList = ref([
  ...
  { value: 'receiverMobile', label: '收件人手机' }
])
/**
 * 聚合搜索切换查询对象时触发
 * @param val
...
onMounted(async () => {
  if (route.query.status!=undefined){
    queryParams.value.status =parseInt(route.query.status)
  }
  await getList()
