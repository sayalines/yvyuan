...
export interface Spu {
  ...
  isLimitTime?: boolean // 是否限时购买
  limitTimeStart?: Date // 限时开始时间
  limitTimeEnd?: Date // 限时结束时间
  giveGift?: number // 赠送礼品
  limitCount?: number // 限购数量
  isFirstProduct?: boolean // 是否首单优惠商品
...
export const getTree = async (params) => {
  return request.get({ url: '/product/spu/tree', params })
}

// 获得商品规格列表
export const getSkuList = (id: number) => {
  return request.get({ url: `/product/spu/sku-list?id=${id}` })
}

// 删除规格前校验 Spu
export const validSkuBeforeDelete = (id: number) => {
  return request.put({ url: `/product/spu/valid-sku-for-delete?id=${id}` })
}
