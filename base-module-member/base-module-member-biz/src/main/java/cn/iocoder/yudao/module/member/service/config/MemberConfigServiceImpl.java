...
public class MemberConfigServiceImpl implements MemberConfigService {
    ...
    public void saveConfig(MemberConfigSaveReqVO saveReqVO) {
        ...
        if (dbConfig != null) {
            MemberConfigDO memberConfigDO = MemberConfigConvert.INSTANCE.convert(saveReqVO).setId(dbConfig.getId());
            memberConfigMapper.updateById(memberConfigDO);
            return;
        }
        // 不存在，则进行插入
        MemberConfigDO memberConfigDO = MemberConfigConvert.INSTANCE.convert(saveReqVO);
        memberConfigMapper.insert(memberConfigDO);
    }
