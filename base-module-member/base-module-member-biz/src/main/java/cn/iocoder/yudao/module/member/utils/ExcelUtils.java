...
public class ExcelUtils {
    ...
    private static <T> T getBean(Class<T> c, JSONObject obj, Map<Integer, String> uniqueMap) throws Exception {
        ...
        for (Field field : fields) {
            ...
            // 是否需要设置异常信息
            if (name.equals(row_tips)) {
                hasRowTipsField = true;
                continue;
            }
            // 原始数据
            if (name.equals(row_data)) {
                field.setAccessible(true);
    ...
    private static <T> void setFieldValue(T t, Field field, JSONObject obj, StringBuilder uniqueBuilder, List<String> errMsgList) {
        ...
        String int_class="int";
        String integer_class="Integer";
        String double_class="double";
        String long_class="long";
        String bigDecimal_class="BigDecimal";
        String date_class="Date";

        try {
            if (string_class.equalsIgnoreCase(fieldClassName)) {
                field.set(t, val);
            } else if (boolean_class.equalsIgnoreCase(fieldClassName)) {
                field.set(t, Boolean.valueOf(val));
            } else if (int_class.equalsIgnoreCase(fieldClassName) || integer_class.equals(fieldClassName)) {
                try {
                    field.set(t, Integer.valueOf(val));
                } catch (NumberFormatException e) {
                    errMsgList.add(String.format("[%s]的值格式不正确(当前值为%s)", cname, val));
                }
            } else if (double_class.equalsIgnoreCase(fieldClassName)) {
                field.set(t, Double.valueOf(val));
            } else if (long_class.equalsIgnoreCase(fieldClassName)) {
                field.set(t, Long.valueOf(val));
            } else if (bigDecimal_class.equalsIgnoreCase(fieldClassName)) {
                field.set(t, new BigDecimal(val));
            } else if (date_class.equalsIgnoreCase(fieldClassName)) {
                try {
    ...
    private static Map<String, JSONArray> readExcelManySheet(MultipartFile mFile, File file) throws IOException {
        return getWorkbookForExcelManySheet(mFile,file);
    }

    private static Map<String, JSONArray> getWorkbookForExcelManySheet(MultipartFile mFile, File file) throws IOException {
        Map<String, JSONArray> map =Collections.emptyMap();
    ...
    private static JSONArray readExcel(MultipartFile mFile, File file) throws IOException {
        return getWorkbookForExcel(mFile,file);
    }

    private static JSONArray getWorkbookForExcel(MultipartFile mFile, File file) throws IOException {
        ...
        try{
            ...
            if (fileName.endsWith(XLSX)) {
                try(Workbook book = new XSSFWorkbook(in)){
                    array = readSheet(book.getSheetAt(0));
                }
            } else if (fileName.endsWith(XLS)) {
                try(POIFSFileSystem poifsFileSystem = new POIFSFileSystem(in)){
                    try(Workbook book = new HSSFWorkbook(poifsFileSystem)){
                        array = readSheet(book.getSheetAt(0));
                    }
                }
    ...
    private static String getCellValue(Cell cell) {
        ...
        if (cell.getCellTypeEnum() == CellType.NUMERIC) {
            ...
            // 去掉尾巴上的小数点0
            String format_str=".*\\.0*";
            if (Pattern.matches(format_str, s)) {
                return s.split("\\.")[0];
            } else {
                return s;
            }
    ...
    private static ExcelClassField getExcelClassField(Field field) {
        ...
        if (!example.isEmpty()) {
            Integer tmp_value = 8;
            if (isNumeric(example) && example.length() < tmp_value) {
                ...
                for (int i = 0; i < sheetDataList.size(); i++) {
                    ...
                    for (int j = 0; j < rowList.size(); j++) {
                        ...
                        try{
                            ...
                            log.error("export cell:{}",e.getMessage());
                            log.info("export rowList:{}",JSONObject.toJSONString(rowList));
                            log.info("export Object:{}",JSONObject.toJSONString(rowList.get(j)));
                        }
                        mergeArray[i][j] = v;
                    }
                }
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        // 是否为字符串
        String row_merge=ROW_MERGE;
        String column_merge=COLUMN_MERGE;
        Integer tmp_value =8;
        if (o instanceof String) {
            String s = o.toString();
            // 当数字类型长度超过8位时，改为字符串类型显示（Excel数字超过一定长度会显示为科学计数法）
            if (isNumeric(s) && s.length() < tmp_value) {
    ...
    private static boolean isNumeric(String str) {
        String blank_value="0.0";
        if (Objects.nonNull(str) && blank_value.equals(str)) {
            return true;
        }
