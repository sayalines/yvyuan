...
public class AreaUtils {
    ...
    public static List<Area> getAreaListByParent(String parentId){
        List<Area> childList = new ArrayList<>();
        List<CsvRow> childRow = rows.stream().filter(row -> row.get(3).equals(parentId)).collect(Collectors.toList());
        for (CsvRow row : childRow) {
            Area area = new Area(Integer.valueOf(row.get(0)), row.get(1), Integer.valueOf(row.get(2)),
                    null, new ArrayList<>());
            childList.add(area);
        }
     ...
     * @return 上级区域编号
     */
    public static String getFormatArea(Integer id, @NonNull AreaTypeEnum type) {
        String result = "";
        if (id!=null){
            while (true){
                Area area = AreaUtils.getArea(id);
                ...
                if (area.getType().equals(type.getType())){
                    result = area.getName();
                    break;
                }
